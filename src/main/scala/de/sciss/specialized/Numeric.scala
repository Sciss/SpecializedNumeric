// same as scala.math.Numeric, however specialized
...
object Numeric {
  trait ExtraImplicits {
     ...
     *  }}}
     */
    implicit def infixNumericOps[@specialized(Int, Float, Long, Double) T](x: T)(implicit num: Numeric[T]): Numeric[T]#Ops = new num.Ops(x)
  }
...
trait Numeric[@specialized(Int, Float, Long, Double) T] extends Ordering[T] {
  def plus(x: T, y: T): T
  def minus(x: T, y: T): T
  def times(x: T, y: T): T
  def negate(x: T): T
