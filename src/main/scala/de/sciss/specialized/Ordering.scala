// same as scala.math.Ordering, however without java.util.Comparator, and specialized
...
trait Ordering[@specialized(Int, Float, Long, Double) T] extends /* Comparator[T] with */ PartialOrdering[T] with Serializable {
  outer =>
...
object Ordering extends LowPriorityOrderingImplicits {
  ...
  trait ExtraImplicits {
     ...
     *  }}}
     */
    implicit def infixOrderingOps[@specialized(scala.Int, scala.Float, scala.Long, scala.Double) T](x: T)(implicit ord: Ordering[T]): Ordering[T]#Ops = new ord.Ops(x)
  }
